...
?>
    <div class="container p-3 m-4 h-100" style="background-color: #FFFFFF; border-top: 5px solid #17337A; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);" >
        ...
        <hr>
        <form action="./verifikasiTransaksiPage.php" method="post">
        ...
        <tbody>
            ...
            if(isset($_POST['search']))
            {
                ...
                WHERE (t.status_transaksi = 'Belum diverifikasi' OR t.status_transaksi = 'Sudah diverifikasi') AND (c.nama LIKE '%$search%' OR p.nama_pegawai LIKE '%$search%' OR m.nama_mobil LIKE '%$search%' OR pr.kode_promo LIKE '%$search%' OR t.status_transaksi LIKE '%$search%') order by t.id_transaksi") or die(mysqli_error($con));
            }
            else
                $query = mysqli_query($con, "SELECT t.format_transaksi, t.id_transaksi, c.format_customer, c.id_customer, c.nama, p.nama_pegawai, m.nama_mobil, d.nama_driver, pr.kode_promo, c.no_telp_customer, t.status_transaksi FROM customer c 
                JOIN transaksi t ON (c.id_customer = t.id_customer) JOIN mobil m ON (m.id_mobil=t.id_mobil) LEFT JOIN driver d ON (d.id_driver=t.id_driver) LEFT JOIN promo pr ON (pr.id_promo=t.id_promo) LEFT JOIN pegawai p ON (p.id_pegawai=t.id_pegawai)
                WHERE (t.status_transaksi = 'Belum diverifikasi' OR t.status_transaksi = 'Sudah diverifikasi') order by t.id_transaksi") or die(mysqli_error($con));
                ...
                echo'
                    <tr>
                        ...
                        <td style="padding-left: 0px !important">';
                            if($data[10]=='Belum diverifikasi'){
                                echo '
                                <form action="../process/editVerifikasiProcess.php?id='.$data[1].'" method="post">
                                    <button name="transaksi">
                                        <a onClick="return confirm ( \'Apakah anda yakin mengverifikasi transaksi ini?\')"><i style="color: green" class="fa fa-edit"></i></a>
                                    </button>
                                </form>
                                <a href="./batalTransaksiPage.php?id='.$data[1].'" style="margin-left:6px"
