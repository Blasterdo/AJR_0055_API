...
?>
    <div class="container p-3 m-4 h-100" style="background-color: #FFFFFF; border-top: 5px solid #17337A; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);" >
        ...
        <tbody>
            ...
            {
                $search = $_POST['search'];
                $query = mysqli_query($con, "SELECT * FROM promo WHERE status_promo!='Kadaluarsa' AND (kode_promo LIKE '%$search%' OR jenis  LIKE '%$search%')") or die(mysqli_error($con));
            }
            else{
                $query = mysqli_query($con, "SELECT * FROM promo WHERE status_promo!='Kadaluarsa'") or die(mysqli_error($con));
            }
                ...
                echo'
                    <tr>
                        ...
                        <td style="padding-left: 0px !important">
                            <a href="./editPromoPage.php?id='.$data[0].'" onClick="return confirm ( \'Yakin mengedit data?\')"><i style="color: green" class="fa fa-edit"></i></a>
